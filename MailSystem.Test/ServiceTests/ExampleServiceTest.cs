...
namespace Sample.Test.ServiceTests
{
    public class ExampleServiceTest
    {
        ...
        public void Setup()
        {
            ...
            repository.Setup(r => r.Get()).Returns(DeliveryMockResult.Get());
            repository.Setup(r => r.Get(It.IsAny<long>())).Returns(DeliveryMockResult.Get().First());
            repository.Setup(r => r.Create(It.IsAny<List<ExampleModel>>())).Returns(1);
            repository.Setup(r => r.Modify(It.IsAny<ExampleModel>())).Returns(DeliveryMockResult.Get().First());
